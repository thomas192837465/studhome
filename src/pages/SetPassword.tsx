...
const roleHome: Record<ProfileRow["role"], string> = {
  etudiant: "/home",
  proprietaire: "/proprietaire/tableau-de-bord",
  admin: "/admin/tableau-de-bord",
  superadmin: "/superadmin/tableau-de-bord",
};

// Landing page for the "mot de passe oublié" email link on the student and
// owner login forms — also covers accounts created before passwords existed
// (they were only ever verified by email OTP), letting them set one for the
// first time.
export function SetPassword() {
  ...
  useEffect(() => {
    // The recovery link redirects here with the session already established
    // by supabase-js (it parses the URL hash automatically).
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(role ? roleHome[role] : "/home");
    } catch {
      ...
      <div className="w-full max-w-md rounded-3xl border border-gray-100 shadow-xl p-8 sm:p-10">
        ...
        {!hasSession ? (
          <p className="mt-4 text-center text-sm text-gray-500">
            Ce lien n'est plus valide ou a expiré. Demandez une nouvelle réinitialisation depuis la page de connexion.
          </p>
        ) : (
